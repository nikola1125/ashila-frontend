...
const ScrollToTopButton = () => {
  ...
  return (
    <button
      ...
      className="fixed bottom-4 right-4 z-[9999] bg-white/80 hover:bg-white/90 text-[#5A3F2A] p-3 rounded-full transition-all duration-300 transform hover:scale-110 hover:-translate-y-1 active:scale-95 flex items-center justify-center min-h-[44px] min-w-[44px]"
      aria-label="Scroll to top"
      style={{ aspectRatio: '1/1' }}
    >
      <ChevronUp size={20} className="text-[#5A3F2A] transition-transform duration-300 hover:scale-110" />
    </button>
  );
};
